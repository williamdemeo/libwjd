/** DFTestModelType */
enum DFTest_ModelType{ DFTest_AR, DFTest_ARD };
/* So far, we only have two simple models available:
   DFTest_AR  <--> basic AR(1) test for unit root:       DY_t =     b Y_{t-1} + u_t
   DFTest_ARD <--> AR(1) test for unit root with drift:  DY_t = a + b Y_{t-1} + u_t

 We should improve this, and code up the so-called "augmented Dickey-Fuller test"
 which accommodates AR(p) models with and without drift and/or trend.
*/

/** dftest() -- test a vector of time-series data for a unit root (non-stationarity).
 * @param N       Length of the vector storing the time-series data to be tested for a unit root.
 * @param y       Vector of time-series data to be tested for a unit root.
 * @param Model   Model type assumed under the null hypothesis.
 * @param alpha   Level of significance at which to test (make this defaut to 0.05)   <<< DEBUG THIS
 * @param testH   Result of the test.  testH=1 means we can reject the null hypothesis of unit root.
 * @param pValue  Probability of observing the computed test statistic under the null hypothesis.
 */
void dftest(const long& N, const double* y, const DFTest_ModelType& Model, double& alpha, int& testH, double& pValue);

/** runTest() -- Test the statistic
@param alpha       The level of significance at which to test.  (make this defaut to 0.05)   <<< DEBUG THIS
@param testStat    On exit, this will contain the value of the test statistic, which is 
                   (beta - 1)/SE(beta), where beta is the coefficient of y_{t-1} in the AR model.
@param sampleSize  The effective sample size, after lags accounted for.  
                   e.g. in AR(1) model, sampleSize = N-1, where N = length(y).
@param Model       Specifies if we're assuming AR or ARD model.
@param testH       On exit, this will be 1 if the test results in rejection of the null hypothesis.
                   Otherwise, testH=0. Recall, the null hypothesis is beta==1 (unit root).
@param pVal        On exit, the p-value = the probability of obtaining the given test stat under the null.
*/
...
/** difference(const long& N, double *x)
 * @param N  length of the vector x
 * @param x  on entry, a vector of length N
...
/** DFTest_CVTables
 ...
 N.B. Different tables are used, depending on the model assumption, as specified by the 
 variable DFTest_ModelType.
*/
const double DFTest_sigLevel[] = { 0.001, 0.005, 0.010, 0.015, 0.020, 0.025, 0.030, 0.035, 0.040, 0.045, 0.050, 0.055, 0.060, 0.065, 0.070, 
...
const int DFTest_sigLevelLength = 50;

// getSigIndex() -- get index of the element in DFTest_sigLevel array that is the greatest value not above alpha.
int getSigIndex(const double& alpha, int& i);


const int DFTest_CVSampleSize[] = {10, 15, 20, 25, 30, 40, 50, 75, 100, 150, 200, 300, 500, 1000, 10000};
const int DFTest_CVSampleSizeLength = 15;

// getSampleSizeIndex() -- get the index of the element in DFTest_CVsampleSize array
//                       that is the greatest value not above sampleSize.
int getSampleSizeIndex(const int& sampleSize, int &i);

const int DFTest_CVTableNumCols = DFTest_sigLevelLength;
const int DFTest_CVTableNumRows = DFTest_CVSampleSizeLength;
const int DFTest_CVTableLength = DFTest_CVTableNumCols*DFTest_CVTableNumRows;

// Note: the table is stored in ROW-major format!
const double DFTest_CVTable_AR[] = {
  ...
  -1.6744, -1.6427, -1.6126, -1.5839, -1.4569, -1.3501, -1.2567, -1.1730,  0.4848,  0.5897,  0.7056,  0.8373,  0.9913,  1.0258,  1.0621,  1.0996,  1.1392,   
   1.1810,  1.2254,  1.2722,  1.3230,  1.3775,  1.4361,  1.5004,  1.5720,  1.6520,  1.7437,  1.8514,  1.9818,  2.1485,  2.3836,  2.7840,  3.7494,
  ...
  -1.6841, -1.6537, -1.6248, -1.5970, -1.4738, -1.3690, -1.2773, -1.1950,  0.4548,  0.5589,  0.6733,  0.8024,  0.9521,  0.9856,  1.0203,  1.0568,  1.0946,
