function [h,pValue,stat,cValue,reg] = pptest(y,varargin)
%PPTEST Phillips-Perron test for a unit root
%
% Syntax:
%
%   [h,pValue,stat,cValue,reg] = pptest(y)
%   [h,pValue,stat,cValue,reg] = pptest(y,param1,val1,param2,val2,...)
%
% Description:
%
%   Phillips-Perron tests assess the null hypothesis of a unit root in a
%   univariate time series y. All tests use the model:
%
%       y(t) = c + d*t + a*y(t-1) + e(t)
%
%   The null hypothesis restricts a = 1. Variants of the test, appropriate
%   for series with different growth characteristics, restrict the drift
%   and deterministic trend coefficients, c and d, respectively, to be 0.
%   The tests use modified Dickey-Fuller statistics (see ADFTEST) to
%   account for serial correlations in the innovations process e(t).
%
% Input Arguments:
%
...
%
%   'test'      String or cell vector of strings indicating the test
%               statistic. Values are 't1' or 't2'. The default is 't1'.
%
%               o When the value is 't1', a modification of the standard t
%                 statistic
%
%                   t1 = (a-l)/se
%
%                 is computed from OLS estimates of the AR(1) coefficient a
%                 and its standard error se in the alternative model.
%
%               o When the value is 't2', a modification of the
%                 "unstudentized" t statistic
%
%                   t2 = T*(a-1)
%
%                 is computed from an OLS estimate of the AR(1) coefficient
%                 a in the alternative model. T is the effective sample
%                 size, adjusted for lag and missing values.
%
%               Tests assess the significance of the restriction a = 1.
%               Statistics are modified to account for serial correlations
...
%       the number of tests. Values are for left-tail probabilities.
%
...
%       size        Effective sample size, adjusted for lag*
...
%       yMu         Mean of y, adjusted for lag*
%       ySigma      Standard deviation of y, adjusted for lag*
%       yHat        Fitted values of y, adjusted for lag*
...
%
%       *Lagging a time series reduces the sample size. Absent a presample
%       value, if y(t) is defined for t = 1:N, then the lagged series
%       y(t-1) is defined for t = 2:N. The effective sample size on the
%       common time base is N-1.
%
% Notes:
%
...
%     more lags. Another method is to look at sample autocorrelations of
%     y(t)-y(t-1); slow rates of decay require more lags. The Newey-West
%     estimator is consistent if the number of lags is O(T^(1/4)), where T
%     is the effective sample size, adjusted for lag and missing values.
%     White and Domowitz [9] and Perron [5] provide further guidelines.
%
...
%
%   o Phillips-Perron statistics follow nonstandard distributions under the
...
%     are interpolated from the tables. Tables are identical to those for
%     ADFTEST for tests of type 't1' and 't2'.
%
% Example:
%
%   % Test GDP data for a unit root using a trend-stationary alternative 
%   % with 0, 1, and 2 lags for the Newey-West estimator:
%
%   load Data_GDP
%   y = log(Data);
%   h = pptest(y,'model','TS','lags',0:2)
%
%   % The test fails to reject the unit-root null with each estimator.
%
% References:
%
...
%
%   [2] Elder, J., and P. E. Kennedy. "Testing for Unit Roots: What Should
...
%
%   [4] Newey, W. K., and K. D. West. "A Simple Positive Semidefinite,
...
%       Dynamics and Control. Vol. 12, 1988, pp.297-332.
%
...
%       Series Regression." Biometrika. Vol. 75, 1988, pp. 335-346.
%
%   [8] Schwert, W. "Tests for Unit Roots: A Monte Carlo Investigation."
%       Journal of Business and Economic Statistics. Vol. 7, 1989,
%       pp. 147-159.
%
%   [9] White, H., and I. Domowitz. "Nonlinear Regression with Dependent
%       Observations." Econometrica. Vol. 52, 1984, pp. 143-162.
%
% See also ADFTEST, KPSSTEST, LMCTEST, VRATIOTEST.

% Copyright 2009-2010 The MathWorks, Inc.
% $Revision: 1.1.6.12 $ $Date: 2010/10/08 16:41:27 $
...
maxT = max(max(sampSizes),T); % Maximum effective sample size
...
if any(T < minT)
    ...
    error(message('econ:pptest:EffectiveSampleSizeLessThanTabulatedValues', minT))    

end

if any(alpha < minAlpha) || any(alpha > maxAlpha)
    
    error('econ:pptest:AlphaOutOfRange',...
...
for i = 1:numTests
    ...
    if ~sameStat % Recompute the statistic only if it changes
        ...
        testReg = runReg(y,testLags,testModel,needRegOut);
        ...
        [testStat,testPValue] = getStat(i,T,testType,testReg,sigLevels,sampSizes,CVTable,needPValue);
    
    end
    
    % Test the statistic:
    
    [testCValue,testH] = runTest(sigLevels,sampSizes,T,testAlpha,testStat,CVTable);
...
function OK = yCheck(y)
    ...
    if isempty(y)
        ...
        error(message('econ:pptest:DataUnspecified'))
          
    elseif ~isnumeric(y)
        
        error(message('econ:pptest:DataNonNumeric'))
          
    elseif ~isvector(y)
        
        error(message('econ:pptest:DataNonVector'))
...
function OK = lagsCheck(lags)
    ...
    if ~isnumeric(lags)
        ...
        error(message('econ:pptest:LagsNonNumeric'))
          
    elseif ~isvector(lags)
        
        error(message('econ:pptest:LagsNonVector'))
          
    elseif any(mod(lags,1) ~= 0) || any(lags < 0)
        
        error(message('econ:pptest:LagsNonNonnegativeInteger'))
...
function OK = modelCheck(model)
    ...
    if ~isvector(model)
        ...
        error(message('econ:pptest:ModelNonVector'))
          
    elseif isnumeric(model) || (iscell(model) && any(cellfun(@isnumeric,model)))
        
        error(message('econ:pptest:ModelNumeric'))
    
    elseif ~all(ismember(upper(model),{'AR','ARD','TS'}))
        
        error(message('econ:pptest:ModelInvalid'))
...
function OK = testCheck(test)
    ...
    if ~isvector(test)
        ...
        error(message('econ:pptest:TypeOfTestNonVector'))
          
   elseif isnumeric(test) || (iscell(test) && any(cellfun(@isnumeric,test)))
        
        error(message('econ:pptest:TypeOfTestNumeric'))
    
    elseif ~all(ismember(upper(test),{'T1','T2'}))
        
        error(message('econ:pptest:TypeOfTestInvalid'))
...
function OK = alphaCheck(alpha)
    ...
    if ~isnumeric(alpha)
        ...
        error(message('econ:pptest:AlphaNonNumeric'))
          
    elseif ~isvector(alpha)
        
        error(message('econ:pptest:AlphaNonVector'))
...
for i = 1:(nargin-1)
    ...
    for j = (i+1):nargin
        ...
        if (paramLength.(iname) > 1) && (paramLength.(jname) > 1) ...
            ...
            error(message('econ:pptest:ParameterSizeMismatch', iname, jname))
...
function testReg = runReg(y,testLags,testModel,needRegOut)
...
switch upper(testModel)
    ...
    % Model: y(t) = c + a*y(t-1) + e(t)
        
        % Design matrix:
        
        X = [ones(T,1),yLag(2:end)];
    ...
    % Model: y(t) = c + d*t + a*y(t-1) + e(t)
        
        % Design matrix:
        
        X = [ones(T,1),(1:T)',yLag(2:end)];
...
%-------------------------------------------------------------------------
% Compute the statistic:
function [testStat,testPValue] = getStat(i,T,testType,testReg,sigLevels,sampSizes,CVTable,needPValue)

% Follows Hamilton [3] p. 514.

aId = strncmp('a',testReg.names,1);
a = testReg.coeff(aId);
se = sqrt(diag(testReg.Cov));
se_a = se(aId);
MSE = testReg.MSE;
gamma0 = testReg.autoCov(1);
lambdaSq = testReg.NWEst;

switch upper(testType)
    
    case 'T1'
        
        testStat = sqrt(gamma0./lambdaSq)*(a-1)/se_a-(0.5)*(lambdaSq-gamma0)*(1/sqrt(lambdaSq))*T*se_a/sqrt(MSE);
         
    case 'T2'
        
        testStat = T*(a-1)-(0.5)*(((T*se_a)^2 )/MSE)*(lambdaSq-gamma0);
        
end

if needPValue
    testPValue = getPValue(i,sigLevels,sampSizes,CVTable,T,testStat);
else
    testPValue = NaN;
end
...
%-------------------------------------------------------------------------
% Get p-values
function testPValue = getPValue(i,sigLevels,sampSizes,CVTable,T,testStat)
...
CVTableRowT = interp2(sigLevels,sampSizes,CVTable,sigLevels,T,'linear');

if testStat <= CVTableRowT(1)
    
    testPValue = sigLevels(1);
    
    warning('econ:pptest:LeftTailStatTooSmall',...
...
elseif testStat >= CVTableRowT(end)
	...
	warning('econ:pptest:LeftTailStatTooBig',...
...
%-------------------------------------------------------------------------
% Test the statistic:
function [testCValue,testH] = runTest(sigLevels,sampSizes,T,testAlpha,testStat,CVTable) 

testCValue = interp2(sigLevels,sampSizes,CVTable,testAlpha,T,'linear');
